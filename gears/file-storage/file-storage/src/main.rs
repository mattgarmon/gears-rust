//! Standalone out-of-process (Profile 3) entrypoint for the `file-storage` gear.
//!
...
/// `file-storage` OoP gear.
#[derive(Parser)]
#[command(name = "file-storage-oop")]
#[command(about = "file-storage - authenticated + DB OoP gear (Profile 3)")]
...
async fn main() -> anyhow::Result<()> {
    ...
    let opts = OopRunOptions {
        gear_name: "file-storage".to_owned(),
